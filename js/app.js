...
function footer() {
  ...
  return '<footer><div class="wrap"><div class="foot-top">'
    ...
    + '<div class="foot-contact"><a href="mailto:julianietokeiner@gmail.com">julianietokeiner@gmail.com</a><br><a href="#" target="_blank" rel="noopener">LinkedIn — Julia Nieto Keiner</a><br><span style="color:#A9BBC6">Kalverstraat, Amsterdam</span></div>'
    + '</div><div class="foot-bottom"><span>© ' + new Date().getFullYear() + ' Julia Nieto</span>'
    + (admin ? '<span style="color:#A9BBC6">Signed in as admin</span>' : '<button class="foot-admin" data-a="open-login">Admin</button>')
    + '</div></div></footer>';
}
...
async function boot() {
  session = await api.getSession();
  api.onAuthChange((s) => { session = s; render(); });
  cats = await api.getCategories();
